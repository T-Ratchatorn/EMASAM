...
class Log:
    def __init__(self, log_each: int, initial_epoch=0):
        self.loading_bar = LoadingBar(length=27)
        self.best_accuracy = 0.0
        self.log_each = log_each
        self.epoch = initial_epoch -1 #-1 because it will be +1 at the start of train function
        self.initial_epoch = initial_epoch -1 #-1 because it will be +1 at the start of train function

    def train(self, step_per_epoch: int) -> None:
        self.epoch += 1
        ...
        self._reset(step_per_epoch)

    def eval(self, step_per_epoch: int) -> None:
        self.flush()
        self.is_train = False
        self._reset(step_per_epoch)
    ...
    def _train_step(self, model, loss, accuracy, learning_rate: float) -> None:
        ...
        if self.step % self.log_each == self.log_each - 1:
            ...
            self.last_steps_state = {"loss": 0.0, "accuracy": 0.0, "steps": 0}
            progress = self.step / self.step_per_epoch
    ...
    def _reset(self, step_per_epoch: int) -> None:
